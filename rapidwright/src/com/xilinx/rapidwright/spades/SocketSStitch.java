...
//  public static int URTLY = 331;
  ...
  public static int LLCRX[] = {4, 4, 4, 4, 4};
  public static int URCRX[] = {6, 8, 6, 6, 6};
  public static int LLCRY[] = {2, 2, 2, 1, 4};
  public static int URCRY[] = {2, 2, 3, 3, 4};
  public static int LLTLX[] = {52, 52, 52, 52, 52};
  public static int URTLX[] = {74, 97, 74, 74, 74};
  public static int LLTLY[] = {100, 100, 100, 4, 288};
  public static int URTLY[] = {191, 191, 287, 287, 331};

  public static boolean isTileValid(Tile tile, int implId) {
    int x = tile.getTileXCoordinate();
    int y = tile.getTileYCoordinate();
    if ((x >= LLTLX[implId]) && (x <= URTLX[implId]) &&
        (y >= LLTLY[implId]) && (y <= URTLY[implId]))
      return true;

    return false;
  }

  public static boolean isCRValid(ClockRegion cr, int implId) {
    if (((cr.getInstanceX() >= LLCRX[implId])  &&
         (cr.getInstanceX() <= URCRX[implId])) &&
        ((cr.getInstanceY() >= LLCRY[implId])  &&
         (cr.getInstanceY() <= URCRY[implId])))
      return true;
  ...
  public static void main(String[] args) throws FileNotFoundException {
    CodePerfTracker t = new CodePerfTracker("SocketS Stitch");
    t.start("SocketS Stitch");
    ...
    for (EDIFPortInst epi : socketCLCell.getCellInst("i_0").getPortInsts()) {
      EDIFNet enet = epi.getNet();
      enet.removePortInst(epi);
    }
    ...
    socketCLCell.removeCellInst(socketCLCell.getCellInst("i_0"));

    //System.out.println(socketCLCell.getCellInst("i_0"));
    ...
    for (Net net : design.getNets()) {
      ...
      for (PIP pip : net.getPIPs()) {
        if (!IsClockNet) {
        if (pip.getTile().getName().contains("INT_X") && !isTileValid(pip.getTile(), implId)) {
    ...
    for (Net net : cl_design.getNets()) {
      ...
      for (PIP pip : net.getPIPs()) {
        ...
        if (IsClockNet) {
          ...
          if ((x == 65 || x == 66 || x == 67 || x == 69 || x == 71 || x == 72 || x == 73) &&
              (pip.getTile().getName().contains("CLE_BC_CORE") || pip.getTile().getName().contains("RCLK_CLE_CORE"))) {
    ...
    //while (cnt == 1) {
    while (!isTileValid(clkPIP.getTile(), implId)) {
    ...
    socket_design.writeCheckpoint("../checkpoints/socket_rw_full_v" + implId + ".dcp");
    EDIFTools.writeEDIFFile("../checkpoints/socket_rw_full_v" + implId + ".edf", socket_netlist, socket_design.getPart().getName());

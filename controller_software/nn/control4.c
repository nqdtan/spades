...
void sync_with_linear_cores() {
  for (int c = 1; c < LINEAR_NUM_CORES; c++) {
    while (SYNC(CORE_ID + c) == 0);
    SYNC(CORE_ID + c) = 0;
  }

  long long int socket_addr = SOCKET5_NOC_ADDR;
  for (int c = 1; c < LINEAR_NUM_CORES; c++) {

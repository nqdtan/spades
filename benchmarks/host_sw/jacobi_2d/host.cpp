...
int main(int argc, char *argv[]) {
  ...
  DATATYPE *data = new DATATYPE [2 * MAT_SIZE];
  ...
  xrt::bo data_buf = xrt::bo(device, 2 * MAT_SIZE * sizeof(DATATYPE), xrt::bo::flags::normal, 0);
...
//  for (int i = num_sockets - 1; i >= 0; i--) {
  ...
  gettimeofday(&tstart, 0);
  jacobi_2d_baseline(gold, gold + MAT_SIZE, MAT_DIM, num_iters);
  ...
  for (int i = 0; i < 2 * MAT_SIZE; i++) {
    if (data[i] != gold[i]) {
      num_mismatches += 1;
      //std::cout << std::dec << i << ": " << std::hex << data[i] << " " << gold[i] << '\n';
    }
  }
